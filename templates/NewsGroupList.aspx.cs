...
namespace development.Templates
{
	...
	{
		protected LinkButton	Subscribe;
		protected Panel			PersonalSettings;
		protected HtmlTableCell CreateEntry;
		protected PageTree		Pagedatatree1;

		private void Page_Load(object sender, System.EventArgs e)
		{
			if(CurrentUser.UserData == null)
				PersonalSettings.Visible = false;
			else if(!Subscription.IsSubscribingTo(CurrentNewsGroup))
		...
		protected void ChangeSubscribtion(object sender, System.EventArgs e)
		{
			if(! CurrentPage.Property.Exists("EPSUBSCRIBE") || (CurrentUser.UserData == null))
				return;
			if(!Subscription.IsSubscribingTo(CurrentNewsGroup))
			{
				Subscribe.Text = Translate("/templates/newsgrouplist/unsubscribe");
				Subscription.SubscribeTo(CurrentNewsGroup);
			}
			else
			{
				Subscribe.Text = Translate("/templates/newsgrouplist/subscribe");
				Subscription.UnSubscribe(CurrentNewsGroup);
			}
		}
