<%@ Control Language="c#" AutoEventWireup="false" Codebehind="News.ascx.cs" Inherits="development.Templates.Workrooms.Templates.Units.News" TargetSchema="http://schemas.microsoft.com/intellisense/ie5" %>
...
<div class="workroomoverviewcolumn leftfloating">
		...
		<episerver:PageList Runat="server" ID="PageList" PageLink='<%#CurrentWorkroom.NewsListRoot%>' Paging="true" PagesPerPagingItem="8">
			<HeaderTemplate>
				<div class="WorkroomList">
					<%ResetRows();%>
					<div class="WorkroomListHeader">
						<%=Translate("plugins/news/displayname")%>
					</div>
			</HeaderTemplate>
			<ItemTemplate>
		...
		<div class="fullwidth">
			<div>
				...
				<div class="leftfloating">
					...
					<asp:RequiredFieldValidator Runat="server" ControlToValidate="PageName" ErrorMessage="*" ID="Requiredfieldvalidator1" NAME="Requiredfieldvalidator1"/>
				</div>
			</div>
			<br />
			<br />
			...
			<div>
				...
				<div class="leftfloating">
					<asp:TextBox Runat="Server" TextMode="MultiLine" Rows="20" Height="10em" CssClass="fullwidth inputarea" ID="MainBody" />
					<asp:RequiredFieldValidator Runat="server" ControlToValidate="MainBody" ErrorMessage="*" ID="Requiredfieldvalidator3" NAME="Requiredfieldvalidator3"/>
				</div>
			</div>
		</div>
		<br />
